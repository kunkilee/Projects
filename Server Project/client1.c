...
int main(int argc, char* argv[]) {
	...
	int server_direct_in = shmget(111, MSGSZ * sizeof(char), IPC_CREAT | 0666);
	if(server_direct_in < 0) {
		printf("shmget error 1\n");
		exit(1);
	}

	int * server_direct_in_memory = (int *) shmat(server_direct_in, NULL, 0); //attach to shared memory for client 1
	...
	while(1) {
		...
		if(msgrcv(status, (void *)&msg, sizeof(msg), 0, 0) < 0) //allows us to take in any type of messages from message queues
		{
			perror("ERROR");
		}
		if(msg.mtype == 1)//when message type is a 1 to 1 message
		{
			...
			rmsg.mtype = 4;
			if(msgsnd(status, (void *)&rmsg, strlen(rmsg.mtext)+1,0) < 0){ //send user's response
					perror("ERROR");
			}
		}
